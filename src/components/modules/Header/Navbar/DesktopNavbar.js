...
export default function DesktopNavbar() {
	...
	return (
		<ul onMouseLeave={mouseLeaveHandler} className="flex items-center justify-center gap-5 xl:gap-10 font-MorabbaBold text-[18px] text-dark-primary">
			<li className="absolute top-[70px] left-0 h-[2.5px] bg-primary bottom-0 transition-all duration-300" style={styles}></li>
			{menuList.map((item) => (
				<motion.li key={item.id} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.8 }} transition={{ type: "spring", stiffness: 300 }} className="cursor-pointer hover:text-primary transition-colors" onMouseEnter={mouseEnterHandler}>
